...
def auto_send_daily_reports():
    """
    Автоматическая отправка отчётов каждый рабочий день в 08:30 (Новокузнецк).
    
    В понедельник отправляется отчёт за пятницу.
    В остальные дни - за предыдущий рабочий день.
    """
    ...
    while True:
        try:
            ...
            # Проверяем: сегодня рабочий день?
